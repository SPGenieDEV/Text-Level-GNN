...
class TextLevelGNNXClass:  # This class is used to achieve parameters sharing final input text
    def __init__(self, test_filename, tokenizer, MAX_LENGTH=10, p=2, min_freq=2):
        ...
        self.label_dict = dict(
            zip(self.test_data[0].unique(), pd.get_dummies(self.test_data[0].unique()).values.tolist()))
    ...
    def build_vocab(self):
        vocab_list = [sentence.split(' ') for _, sentence in self.test_dataset]
        unique_vocab = []
...
class TextLevelGNN(nn.Module):
    def __init__(self, pretrained_embeddings, out_size=10, dropout_rate=0, padding_idx=1):
...
model = TextLevelGNN(pretrained_embeddings=torch.tensor(dataset_input.embedding_matrix), dropout_rate=0).to(device)
...
# Prepare input text as Dataloader
...
for i, (node_sets, neighbor_sets, public_edge_masks, labels) in enumerate(input_text_loader):
...
test_accuracy = test_correct_items / len(dataset_input.test_dataset)
